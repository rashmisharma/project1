...
print_r($mail_template);
		...
		$body   = str_ireplace("Date of Event : [date]","Date of Event : ".$_REQUEST['date'],$body);
		
		echo $_REQUEST['your-name'];
		exit();
		$additional_headers =
		...
		if($subject=="Download Pricelist Form")
		{
			...
			session_start();
			$_SESSION['download'] = 1;
			$attach_headers = "From: support@photobooth.com"; 
			...
			$attach_headers .= "This is a multi-part message in MIME format.\n\n" . 
			"--{$mime_boundary}\n" . 
			...
			$attach_msg = "Hi,\n\n\We have attached the price list for your reference.";
			
			$ok = @mail($sender, "PhotoBooth Price List", $attach_msg, $attach_headers); 
...
{
	//get mails for last one week
	$query = mysql_query("select body,date_format(created_date,'%M-%d-%Y') from contact_form where created_date >= DATE_SUB(now(),INTERVAL 7 DAY) order by created_date asc;") or die(mysql_error());
