...
class WPCF7_ContactForm {
	...
	function form_response_output() {
			...
			if ( isset( $_POST['_wpcf7_mail_sent'] ) && $_POST['_wpcf7_mail_sent']['id'] == $this->id ) {
				if ( $_POST['_wpcf7_mail_sent']['ok'] ) {
					...
					if(isset($_SESSION['download']) && $_SESSION['download']==1)
					{
							...
							if($open==2)
							{
								$content = "Thank you for your request. The price list has also been emailed to you.";
							}
		...
		if($open==2)
		{
			return '<div' . $class . '>' . $content . '</div><script type="text/javascript">
window.open("Photo_Booth_Pro_PriceList.pdf");
</script>';
		}
	}
	...
	function compose_and_send_mail( $mail_template ) {
		...
		$body   = str_ireplace("[date]",$_REQUEST['date'],$body);
		//echo $body;
		$additional_headers =
		...
		if($subject=='"Download Pricelist Form"')
		{
			$type = 2;
			session_start();
			$_SESSION['download'] = 1;
			
			$fileatt = "Photo_Booth_Pro_PriceList.pdf";
			$file = fopen($fileatt,'rb'); 
			$data = fread($file,filesize($fileatt)); 
			fclose($file); 
			
			$to = $sender; 
			$from = "support@photoboothpro.com";
			$subject = "Photobooth Price List"; 
			$message = "<p>Hi,<br><br>Please see the attachment for price list.</p>";
			...
			$attach_headers  = "From: ".$from.$eol;
			$attach_headers .= "MIME-Version: 1.0".$eol; 
			$attach_headers .= "Content-Type: multipart/mixed; boundary=\"".$separator."\"".$eol.$eol; 
			$attach_headers .= "Content-Transfer-Encoding: 7bit".$eol;
			$attach_headers .= "This is a MIME encoded message.".$eol.$eol;
			
			// message
			$attach_headers .= "--".$separator.$eol;
			$attach_headers .= "Content-Type: text/html; charset=\"iso-8859-1\"".$eol;
			$attach_headers .= "Content-Transfer-Encoding: 8bit".$eol.$eol;
			$attach_headers .= $message.$eol.$eol;
			
			// attachment
			$attach_headers .= "--".$separator.$eol;
			$attach_headers .= "Content-Type: application/octet-stream; name=\"".$filename."\"".$eol; 
			$attach_headers .= "Content-Transfer-Encoding: base64".$eol;
			$attach_headers .= "Content-Disposition: attachment".$eol.$eol;
			$attach_headers .= $attachment.$eol.$eol;
			$attach_headers .= "--".$separator."--";
			
			// send message
			mail($to, $subject, "", $attach_headers);
		...
		$val = addslashes($body);
		$query = mysql_query("insert into contact_form(body,form) values(\"$val\",'$type')") or die(mysql_error());
		...
		if($_REQUEST['date']!="")
		{
			//check if Saturday
			//echo $_REQUEST['date'];
			$dayOfWeek = date('N', strtotime($_REQUEST['date']));
			if($dayOfWeek==6)
			{
				$xml = '<xmlrequest>
						<username>admin</username>
						<usertoken>b9ede968e4549eb4f27ee9e57f4e7102ba663694</usertoken>
						<requesttype>subscribers</requesttype>  
     					<requestmethod>AddSubscriberToList</requestmethod>  
     				<details>  
         				<emailaddress>'.$_REQUEST['your-email'].'</emailaddress>  
			             ...
			             <item>  
			                 <fieldid>2</fieldid>  
			                 <value>'.$_REQUEST['first-name'].'</value>  
			             </item>  
			         </customfields>  
			     </details>  
			</xmlrequest>';
		...
		$ch = curl_init('http://photoboothpro.com/emailmarketer/xml.php');  
